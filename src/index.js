...
import type { PackageAtVersion } from './types';
...
function buildGraph(rootPackage: PackageAtVersion, pacoteOptions: Object) {
  return getAllDependencies(rootPackage, pacoteOptions).then(result => (
    getLatestDependencies(
      _.uniq(result.dependencies.map(npmPackage => npmPackage.name)),
    )
    .then(latestVersions => (
